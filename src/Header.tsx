...
export interface HeaderProps {
    ...
    readonly headerImageSmall: ImageDataLike;
}

function renderHeaderImage(headerImage: IGatsbyImageData) {
    return function renderHeaderImageSmall(headerImageSmall: IGatsbyImageData): JSX.Element {
        const images = withArtDirection(headerImage, [
            {
                media: '(max-width: 639px)',
                image: headerImageSmall,
            },
        ]);
        ...
        return (
            <header className="relative">
                <GatsbyImage
                    ...
                    image={images}
                    as="figure"
                    className="bg-gradient-to-b from-dracula-cyan h-64 sm:h-4/5"
                    imgClassName="mix-blend-luminosity object-top sm:object-center"
                />
...
export const Header: FunctionComponent<HeaderProps> = (props) => {
    const headerImage = Maybe.of(getImage(props.headerImage));
    const headerImageSmall = Maybe.of(getImage(props.headerImageSmall));

    return Maybe.just(renderHeaderImage).ap(headerImage).ap(headerImageSmall).unwrapOr(null);
};
