...
test("GitHub Repos", async ({ page }) => {
	...
	const element = await page.waitForSelector('[aria-label="GitHub Repos"]', { timeout: 3000 });
	const text = await element.textContent();
...
test("GitHub Stars", async ({ page }) => {
	...
	await page.waitForLoadState("networkidle");
	const element = await page.waitForSelector('[aria-label="GitHub Stars"]', { timeout: 3000 });
	const text = await element.textContent();
