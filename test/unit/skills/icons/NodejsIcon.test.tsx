...
import { NodejsIcon, NodejsIconProps } from '../../../../src/skills/icons/NodejsIcon';

const nodejsIconPropsFactory = Factory.define<NodejsIconProps>(() => {
    return {};
});

function renderNodejsIcon(propsOverrides?: Partial<NodejsIconProps>): ReactTestRenderer {
    const props = nodejsIconPropsFactory.build(propsOverrides);
    return create(<NodejsIcon className={props.className} />);
}

test('renders an <svg /> element', (t) => {
    const { root } = renderNodejsIcon();
...
test('<svg /> element has a viewbox set', (t) => {
    ...
    t.is(svgElement.props.viewBox, '0 0 256 289');
});
...
test('<svg /> accepts a className', (t) => {
    const props = nodejsIconPropsFactory.build({
        className: 'foo',
    });
    const { root } = renderNodejsIcon(props);
    const svgElement = root.findByType('svg');

    t.is(svgElement.props.className, 'foo');
});

test('<svg /> renders one <path /> element', (t) => {
    const { root } = renderNodejsIcon();
    const svgElement = root.findByType('svg');
    const pathElements = svgElement.findAllByType('path');

    t.is(pathElements.length, 1);
});

test('<path /> element has correct fill color', (t) => {
    const { root } = renderNodejsIcon();
    const svgElement = root.findByType('svg');
    const pathElements = svgElement.findAllByType('path');

    t.is(pathElements.at(0)?.props.fill, '#539E43');
});

test('<path /> element has correct d property set', (t) => {
    const { root } = renderNodejsIcon();
    const svgElement = root.findByType('svg');
    const pathElements = svgElement.findAllByType('path');
