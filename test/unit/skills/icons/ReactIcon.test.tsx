...
import { ReactIcon, ReactIconProps } from '../../../../src/skills/icons/ReactIcon';

const reactIconPropsFactory = Factory.define<ReactIconProps>(() => {
    return {};
});

function renderReactIcon(propsOverrides?: Partial<ReactIconProps>): ReactTestRenderer {
    const props = reactIconPropsFactory.build(propsOverrides);
    return create(<ReactIcon className={props.className} />);
}

test('renders an <svg /> element', (t) => {
    const { root } = renderReactIcon();
...
test('<svg /> element has a viewbox set', (t) => {
    ...
    t.is(svgElement.props.viewBox, '0 0 256 230');
});
...
test('<svg /> accepts a className', (t) => {
    const props = reactIconPropsFactory.build({
        className: 'foo',
    });
    const { root } = renderReactIcon(props);
    const svgElement = root.findByType('svg');
...
test('first <path /> element has correct fill color', (t) => {
    ...
    t.is(pathElements.at(0)?.props.fill, '#FFF');
});
...
test('second <path /> element has correct fill color', (t) => {
    ...
    t.is(pathElements.at(1)?.props.fill, '#53C1DE');
});
...
test('fourth <path /> element has correct fill color', (t) => {
    ...
    t.is(pathElements.at(3)?.props.fill, '#53C1DE');
});
...
test('fourth <path /> element has correct d property set', (t) => {
    ...
    t.is(
        ...
        'M128.221 94.665c11.144 0 20.177 9.034 20.177 20.177 0 11.144-9.033 20.178-20.177 20.178-11.143 0-20.177-9.034-20.177-20.178 0-11.143 9.034-20.177 20.177-20.177',
    );
});
