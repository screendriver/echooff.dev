...
describe("countWebmentions()", () => {
	it("counts reactions and replies as Webmentions", () => {
		const actualWebmentionCount = countWebmentions({
			reactions: {
				bookmarkCount: 1,
				likeCount: 2,
				repostCount: 3
		...
		const expectedWebmentionCount = 7;

		expect(actualWebmentionCount).toBe(expectedWebmentionCount);
	});
});

describe("countHackerNewsPosts()", () => {
	it("counts the number of story mentions", () => {
		const actualHackerNewsPostCount = countHackerNewsPosts({
			mentions: [
				{
					commentCount: 12,
		...
		const expectedHackerNewsPostCount = 2;

		expect(actualHackerNewsPostCount).toBe(expectedHackerNewsPostCount);
	});
});

describe("formatBuildMentionTotals()", () => {
	it("formats the build summary line", () => {
		const actualSummaryLine = formatBuildMentionTotals({
			totalHackerNewsPostCount: 4,
			totalWebmentionCount: 23
		});
		const expectedSummaryLine = "Build mention totals: 23 Webmentions, 4 Hacker News posts";

		expect(actualSummaryLine).toBe(expectedSummaryLine);
	});
});
...
describe("createBuildMentionTotalsRecorder()", () => {
	it("accumulates totals across multiple blog posts and writes the summary once", () => {
		...
		const expectedSummaryLine = "Build mention totals: 5 Webmentions, 1 Hacker News posts";

		const actualWriteLine = writeLine;

		expect(actualWriteLine).toHaveBeenCalledExactlyOnceWith(expectedSummaryLine);
	});
});
